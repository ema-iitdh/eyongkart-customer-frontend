...
const ProductDisplay = (props) => {
  ...
  const { data: productData } = useQuery(
    ["productData", productId], // Unique key for the query
    () => fetchProductData(productId), // The query function
    {
      enabled: !!productId, // Ensure query only runs when productId is valid
      onSuccess: () => setLoading(false), // Set loading state to false when data is fetched
      onError: () => setLoading(false), // Set loading state to false in case of error
    }
  );
  const imgData1 = `${
    CloudinaryConfig.CLOUDINARY_URL
                    ...
                    <div className="productdisplay-right mt-2 sm:mt-0">
                      ...
                      <div className="productdisplay-right-prices">
                        <div className="mt-[-20px]">
                          <span className="text-red-500 text-[18px] sm:text-[24px]">
                            ₹{productData.discountedPrice}
                          </span>
                          <span className="pl-3 line-through sm:text-[20px] text-[16px] opacity-65">
                            ₹{productData.price}
                          </span>
                          <span className="text-emerald-500 text-[14px] sm:text-[18px] ml-2">
